...
export default function TechnicianManager({ technicians, onUpdate }: TechnicianManagerProps) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800 mb-2">Manage Technicians</h2>
      <p className="text-gray-600 mb-6">Add, remove, or deactivate technicians who can complete jobs.</p>
          ...
          technicians.map((tech) => (
            ...
            >
              <div className="flex items-center space-x-4">
                <div className={`w-3 h-3 rounded-full ${tech.active ? 'bg-green-500' : 'bg-gray-400'}`} />
                <div>
                  <p className={`text-lg font-semibold ${tech.active ? 'text-gray-800' : 'text-gray-500'}`}>
                    {tech.name}
                  </p>
                  <p className="text-sm text-gray-500">{tech.active ? 'Active' : 'Inactive'}</p>
                </div>
              </div>
